...
- “基础”维护显示名称、业务 ID、Provider 类型与 Base URL；启用状态在 Provider 目录卡片右侧直接切换。
...
- `OllamaHub.db` 仍是唯一配置真源，但页面不使用独立状态条幅重复展示该信息。
- Provider 表单使用防抖自动保存，目录启停、Model、密钥与 Header 等离散操作立即保存并通过 toast 反馈；监听地址等需要重启的设置不放在本页。
