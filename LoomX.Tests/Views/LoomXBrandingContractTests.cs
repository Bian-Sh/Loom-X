...
public sealed class LoomXBrandingContractTests
{
    ...
    public void 用户可见品牌使用Loomx而不是技术程序集名()
    {
        ...
        Assert.Contains("settings.about.description", settingsView, StringComparison.Ordinal);
        Assert.DoesNotContain("控制中心", settingsView, StringComparison.Ordinal);
        Assert.Contains("settings.local.data.export.content", settingsViewModel, StringComparison.Ordinal);
