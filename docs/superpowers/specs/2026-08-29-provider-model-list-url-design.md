...
- “模型” Tab 在搜索/添加/同步工具栏下方放置 URL 字段：标签为“模型列表 URL（可选）”，占满可用宽度，水印为 `例如：https://api.example.com/v1/models`，辅助说明为“留空时根据 Base URL 自动推导；填写后仅用此地址刷新模型，不影响聊天请求。”。
- API Key 输入使用密码字符掩码，并提供眼睛图标按钮切换显示状态；切换只影响当前控件显示，不改变保存值或 `HasApiKey` 状态。若项目现有 Avalonia 版本不支持内置模板切换，则在 View 代码后置中以同一绑定源实现等价的双控件互斥显示。
