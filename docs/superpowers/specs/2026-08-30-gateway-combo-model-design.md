...
- 模型选择器中的 Provider 分组使用轻量 `>` 作为折叠指示器。指示器位于 header 左侧，展开时顺时针旋转 90 度；header 右侧只显示模型数量，避免与“进入下一层”语义混淆。
- 操作图标按钮必须有 Tooltip 和可访问名称，布局保持稳定；除折叠状态指示器外，不使用字符替代操作图标，也不使用文字删除按钮。
