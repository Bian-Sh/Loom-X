...
public static class LoomXHost
{
    ...
    private static void MapEndpoints(WebApplication app)
    {
        ...
        adminApi.MapPut("/providers/{providerId:guid}/models/order", async (Guid providerId, ConfigurationManagementService service, ModelOrderInput input, CancellationToken cancellationToken) => Results.Ok(await service.UpdateModelOrderAsync(providerId, input, cancellationToken)));
        ...
        adminApi.MapGet("/gateway/combos", (ConfigurationManagementService service, CancellationToken cancellationToken) => service.ListGatewayCombosAsync(cancellationToken));
        adminApi.MapPut("/gateway/{key}/combos", async (string key, GatewayEndpointComboSelectionInput input, ConfigurationManagementService service, CancellationToken cancellationToken) => Results.Ok(await service.UpdateGatewayEndpointComboBindingsAsync(key, input, cancellationToken)));
        adminApi.MapPost("/gateway/combos", async (GatewayComboInput input, ConfigurationManagementService service, CancellationToken cancellationToken) => Results.Ok(await service.CreateGatewayComboAsync(input, cancellationToken)));
