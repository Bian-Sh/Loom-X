...
public sealed class ConfigSnapshotService : IDisposable
{
    ...
    public Task<GatewayEndpointResponse> RotateGatewayApiKeyAsync(string key, CancellationToken cancellationToken = default) => ExecuteManagementAsync((service, token) => service.RotateGatewayApiKeyAsync(key, token), cancellationToken);
    public Task<GatewayEndpointResponse> UpdateGatewayEndpointReasoningEffortAsync(string key, string value, CancellationToken cancellationToken = default) => ExecuteManagementAsync((service, token) => service.UpdateGatewayEndpointReasoningEffortAsync(key, value, token), cancellationToken);
    public Task<GatewayEndpointResponse> UpdateGatewayEndpointComboBindingsAsync(string endpointKey, GatewayEndpointComboSelectionInput input, CancellationToken cancellationToken = default) => ExecuteManagementAsync((service, token) => service.UpdateGatewayEndpointComboBindingsAsync(endpointKey, input, token), cancellationToken);
    public Task<GatewayComboResponse> CreateGatewayComboAsync(GatewayComboInput input, CancellationToken cancellationToken = default) => ExecuteManagementAsync((service, token) => service.CreateGatewayComboAsync(input, token), cancellationToken);
