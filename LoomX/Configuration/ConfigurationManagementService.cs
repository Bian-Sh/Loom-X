...
public sealed record ModelResponse(Guid Id, string ProviderId, string ModelId, string DisplayName, string? ConfigId, string Family, string? BaseUrl, string? ApiMode, int ContextLength, int MaxTokens, bool Vision, double? Temperature, double? TopP, bool Enabled, bool HasApiKey, string HeadersJson, string ExtraJson, string? OwnedBy = null, string? RemoteFamily = null, int? RemoteContextLength = null, int? RemoteMaxTokens = null, bool? RemoteVision = null, int SortOrder = 0);
...
public sealed record GatewayEndpointComboSelectionInput(IReadOnlyList<Guid> ComboIds);
    ...
    bool reloadProviderAfterWrite = true)
{
    ...
    public async Task<AppSettingsResponse> UpdateSettingsAsync(AppSettingsInput input, CancellationToken cancellationToken = default)
    {
        ...
        settings.UseProxyForUpdates = input.UseProxyForUpdates;
        settings.StartWithWindows = input.StartWithWindows;
    ...
    public async Task<AppSettingsResponse> SetGatewayRunningAsync(bool gatewayRunning, CancellationToken cancellationToken = default)
    {
        await using var db = await dbContextFactory.CreateDbContextAsync(cancellationToken);
        var settings = await db.AppSettings.SingleAsync(cancellationToken);
        settings.GatewayRunning = gatewayRunning;
        await db.SaveChangesAsync(cancellationToken);
        await ReloadProviderAsync(cancellationToken);
        return ToResponse(settings);
    }
    ...
    {
        await using var db = await dbContextFactory.CreateDbContextAsync(cancellationToken);
        var models = await db.Models.Where(item => item.ProviderId == providerId).ToListAsync(cancellationToken);
        var requested = input.ModelIds.Distinct().ToArray();
        var byId = models.ToDictionary(item => item.Id);
        var order = 0;
    ...
    {
        await using var db = await dbContextFactory.CreateDbContextAsync(cancellationToken);
        var endpoints = await db.GatewayEndpoints.AsNoTracking()
            .Include(item => item.ComboBindings)
    ...
    {
        await using var db = await dbContextFactory.CreateDbContextAsync(cancellationToken);
        var combos = await db.GatewayCombos.AsNoTracking().AsSplitQuery()
            .Include(item => item.Routes).ThenInclude(item => item.Model).ThenInclude(item => item.Provider)
            .Include(item => item.EndpointBindings).ThenInclude(item => item.Endpoint)
            .OrderBy(item => item.SortOrder).ThenBy(item => item.Name)
            .ToListAsync(cancellationToken);
        return combos.Select(ToGatewayComboResponse).ToArray();
    }
    ...
    {
        await using var db = await dbContextFactory.CreateDbContextAsync(cancellationToken);
        var endpoint = await db.GatewayEndpoints.Include(item => item.ComboBindings).SingleOrDefaultAsync(item => item.Key == endpointKey, cancellationToken) ?? throw new KeyNotFoundException("Endpoint 不存在。");
        var comboIds = input.ComboIds.Distinct().ToArray();
        var combos = await db.GatewayCombos.Where(item => comboIds.Contains(item.Id)).ToDictionaryAsync(item => item.Id, cancellationToken);
        ...
        for (var index = 0; index < comboIds.Length; index++)
        {
            ...
            if (existing.TryGetValue(comboId, out var binding))
            {
    ...
    {
        await using var db = await dbContextFactory.CreateDbContextAsync(cancellationToken);
        _ = await db.GatewayCombos.FindAsync([comboId], cancellationToken) ?? throw new KeyNotFoundException("Combo 模型不存在。");
    ...
    private static Task<GatewayComboEntity?> LoadComboAsync(ConfigurationDbContext db, Guid id, CancellationToken cancellationToken) =>
        db.GatewayCombos.AsSplitQuery()
            .Include(item => item.Routes).ThenInclude(item => item.Model).ThenInclude(item => item.Provider)
            .Include(item => item.EndpointBindings).ThenInclude(item => item.Endpoint)
    ...
    {
        "stable" => "stable",
        _ => throw new ArgumentException("当前仅支持稳定版更新。")
    };
