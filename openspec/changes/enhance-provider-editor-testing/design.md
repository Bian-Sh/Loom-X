...
### 5. 受控的响应展示
测试器允许在 UI 中显示响应正文，但普通和流式累计文本都设置字符上限；超过上限后停止向 UI 追加并标记截断，同时仍正确结束或取消底层请求。非 2xx、无效 JSON 和协议结构错误保留受限长度的原始上游内容，JSON 只进行缩进格式化；流式解析失败保留此前已收到的 JSONL 与当前原始 payload；没有响应体的异常生成安全错误 JSON。日志只记录安全元数据。Response 使用只读可选择文本和系统原生复制能力，请求结束后仅当用户全选全部内容并按 Delete 或 Backspace 时清空，部分选择或请求执行中不清空。由于只读 TextBox 会在冒泡阶段消费删除键，页面在父级隧道路由阶段监听该快捷键并包含已处理事件，确保真实键盘交互能够触发清空。
