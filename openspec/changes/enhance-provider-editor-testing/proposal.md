...
- 流式测试按接收顺序展示所有 SSE `data:` payload：有效 JSON 压缩为单行 JSONL，非 JSON 与 `[DONE]` 原样保留；发送后按钮进入不可中止的动态等待状态。Response 使用无标题、无清空按钮的可选择只读文本，请求结束后可通过全选并按 Delete/Backspace 清空；切换 Provider 时在内部取消未完成测试并重置上下文。
