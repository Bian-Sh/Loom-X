...
Overview 的 Runtime NodeGraph 保留覆盖完整控件区域的 Surface 绘制层，以维持空白区域的滚轮缩放和拖拽平移命中，同时与页面其他 Surface 卡片保持一致。外框使用共享 `BorderBrush` 和 10px 圆角；控件内部绘制也裁剪到同一圆角，避免画布背景填平外层四角，不再使用深色 Graph 边框。
