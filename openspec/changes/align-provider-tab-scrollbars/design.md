...
2. 为四个 Tab 的外层 `ScrollViewer` 添加统一 `provider-tab-scroll` class，并通过页面局部样式设置 `Margin="0,0,20,0"`。这样滚动条位置由一个共享样式控制，且相对面板边缘精确向左 20px。
...
- [默认 Tab 模板变化影响位置] → 契约测试锁定页面自身的几何约束，并通过打包后的实机界面验证最终位置。
